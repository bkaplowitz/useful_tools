...
from .dotdict import dotdict, mapping
...
def tensorify(a):
    ...
    return torch.as_tensor(
        np.array(a), dtype=dtype, device=torch.device("cuda")
    )
...
def stack(x, *args, **kwargs):
    if isinstance(x[0], dict):
        ...
        return x[0].__class__({
            k: stack([y[k] for y in x], *args, **kwargs) for k in ks
        })
...
def cat(x, *args, **kwargs):
    if isinstance(x[0], dict):
        ks = x[0].keys()
        return x[0].__class__({
            k: cat([y[k] for y in x], *args, **kwargs) for k in ks
        })
    if TORCH and isinstance(x[0], torch.Tensor):
...
def _arrdict_factory():
    ...
    methods = {
        f"__{name}__": partialmethod(_arrdict_base.__binary_op__, f"__{name}__")
